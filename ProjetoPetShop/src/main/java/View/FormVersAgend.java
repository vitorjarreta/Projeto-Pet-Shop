 ...
 */
public class FormVersAgend extends javax.swing.JFrame {

    /**
     * Creates new form FormVerprodutos
     */
    public FormVersAgend() {
        initComponents();
        configurarForm();
    }
    ...
    private void bt_pesquisarprodutoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bt_pesquisarprodutoActionPerformed
        if (pesquisar_produto.getText().isEmpty()) {
            preencherTable(new AgendamentoDAO().listar());
        } else {
            preencherTable(new AgendamentoDAO().PesquisarNome(pesquisar_produto.getText()));
        }
    ...
    private void tabela_produtosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabela_produtosMouseClicked
        if(evt.getClickCount() == 2){
            ...
            FormAgendamento f = new FormAgendamento(id,1);
            f.setVisible(true);
            this.dispose();
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FormVersAgend.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new FormVersAgend().setVisible(true);
            }
        });
    }
    ...
    private void configurarForm() {
        this.setTitle("Agendamento cadastrados");
    ...
    private void preencherTable(List<Agendamento> lista){
        if (lista!=null) {
            if (lista.size()>0) {
                configurarTable();
                ...
                for(Agendamento obj: lista){
                    m2.addRow(new Object[]{
                        obj.getId(),
                        new ServicosDAO().pesquisarPorID(Integer.toString(obj.getId_servicos())).getNome(),
                        new ClientesDAO().pesquisarPorID(Integer.toString(obj.getId_clientes())).getNome(),
                        new PetsDAO().pesquisarPorID(Integer.toString(obj.getId_pets())).getNome(),
                        obj.getData()
                    });
                }
